    ...
    if($_POST){
        ...
        $level=$_POST['level'];
            ...
            if(empty($password)){
                ...
                nama_petugas='".$nama_petugas."',level='".$level."',username='".$username."',
                id_petugas='".$id_petugas."' where id_petugas = '".$id_petugas."' ") or die(mysqli_error($conn));
            if($update){
                ...
                $update=mysqli_query($conn,"update petugas set nama_petugas='".$nama_petugas."',level='".$level."',username='".$username."',
                password='".md5($password)."', id_petugas='".$id_petugas."' where id_petugas = '".$id_petugas."'") or die(mysqli_error($conn));
            if($update){
                echo "<script>alert('Sukses update petugas');location.href='tampil_petugas.php';</script>";
            } else {
                echo "<script>alert('Gagal update petugas');location.href='ubah_petugas.php?id_petugas=".$id_petugas."';</script>";
